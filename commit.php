...
switch ($_GET['action']) {
    ...
    switch ($_GET['type']) {
        ...
        case 'people':
            ...
            $actor = isset($_POST['actor']) ?
                trim($_POST['actor']) : '';
            $director = isset($_POST['director']) ?
                trim($_POST['director']) : '';
            if (empty($director) and empty($actor)) {
                $error[] = urlencode('Please select ONE profession.');
            }
            if (empty($error)) {
                $query = 'INSERT INTO
                    people
                        (people_fullname, people_isactor, movie_type, people_isdirector)
                    VALUES
                        ("' . $people_fullname . '",
                         ' . $actor . ',
                         ' . $director . ')';
